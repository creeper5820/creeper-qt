...
namespace pro {
    ...
    using MinimumHeight =
        SetterProp<Token, int, [](auto& self, int v) { self.setMinimumHeight(v); }>;
    using MaximumHeight =
        SetterProp<Token, int, [](auto& self, int v) { self.setMaximumHeight(v); }>;
    ...
    using BaseSize =
        DerivedProp<Token, QSize, [](auto& self, const QSize& v) { self.setBaseSize(v); }>;
    using FixedSize =
    ...
    using WindowIconText = DerivedProp<Token, QString,
        [](auto& self, const QString& v) { self.setWindowIconText(v); }>;
    using WindowRole =
